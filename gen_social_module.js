...
const VisionSubCard = ({
  ...
  const handleCopyAndGo = () => {
    ...
    const textToCopy = ratio === "文字" ? selected : \\\\n--ar \\;
    ...
    try {
      ...
      reader.onloadend = async () => {
        ...
        const res = await fetch("/api/notion/upload-file", {
          ...
          body: JSON.stringify({ pageId, fileData: base64data, fileName: \social_\_\.png\, blockTitle: title }),
        });
        const data = await res.json();
        if (data.error) throw new Error(data.error);
        toast.success(\\ 圖片已成功匯入 Notion！\);
        setImageFile(null);
...
fs.writeFileSync('src/components/SocialVisionModule.tsx', content, 'utf8');
